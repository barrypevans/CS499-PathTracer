Shader "PathTracer_PBR_Generated"
{
	Properties
	{
		...
		SubShader
	{
		...
		Pass
		{
			...
			//#RECUR
			float3 GetIndirectLight(Hit h, Ray ray) {



				float3 direct = GetDirectLight(h, ray);

				Ray irrRay;
				irrRay.origin = h.position;
				irrRay.direction = sampleUnitHemisphereNormal(h.normal);
				Hit irrHit = Intersect(newRay);
				float3 irradiance = GetIndirectLight(irrHit, irrRay);

				Ray specRay;
				specRay.origin = h.position;
				specRay.direction = sampleUnitHemisphereNormal(h.normal);
				Hit specHit = Intersect(newRay);
				float3 specIndirect = GetIndirectLight(specHit, specRay);
				...
				vec3 ambient = (kD * diffuse + kS * specIndirect);
			...
			float3 colorAccum = float3(1,1,1);
				...
				float3 irradiance = float3(.03,.03,.03) * 0;
